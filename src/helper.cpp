...
std::string GenreToStr(Genre genre) {
  if (genre == Genre::BIOGRAPHY)
    return "Biography";
  if (genre == Genre::MEMOIR)
    return "Memoir";
  if (genre == Genre::FANTASY)
    return "Fantasy";
  if (genre == Genre::SCIFI)
    return "Sci-fi";
  if (genre == Genre::YOUNG_ADULT)
    return "Young Adult";
  if (genre == Genre::HISTORICAL_FICTION)
...
std::string StatusToStr(Status status) {
  ...
  if (status == Status::FINISHED)
    return "Finished";
  if (status == Status::DID_NOT_FINISH)
...
std::string ToStandardFormat(std::string word) {
  ...
  for (int i = 0; i < word.size(); i++) {
    ...
    // Capitalize if: first letter, or if the previous letter was a space,
    // opening paren or period
